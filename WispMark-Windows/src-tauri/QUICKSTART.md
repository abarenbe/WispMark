# WispMark Windows - Quick Start Guide
...
- **Portable EXE**: `src-tauri/target/release/WispMark.exe`
...
2. Copy the exe from `target/release/WispMark.exe` to a test folder
3. Create a `.portable` file in the same folder:
   ```bash
   # In the folder with WispMark.exe:
   echo. > .portable
   ```
