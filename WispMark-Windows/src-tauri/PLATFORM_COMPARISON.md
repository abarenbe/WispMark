...
This document maps WispMark features between the macOS (Swift) and Windows (Rust/Tauri) implementations.
...
func applicationSupportDirectory() -> URL {
    FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        .appendingPathComponent("WispMark")
}
...
1. **Locate macOS notes:**
   ```
   ~/Library/Application Support/WispMark/notes.json
   ```
...
3. **Or use portable mode:**
   - Create `.portable` file next to WispMark.exe
