# FloatMD Windows - Tauri Backend Implementation Summary

## Overview

Complete Tauri v2 Rust backend for FloatMD Windows, providing full feature parity with the macOS version plus Windows-specific enhancements.
...
7. **`tauri.conf.json`**
   - App metadata (FloatMD, com.floatmd.app)
...
4. Otherwise → use %APPDATA%/FloatMD/notes.json (standard)
```
...
- **Portable EXE:** `target/release/FloatMD.exe`
